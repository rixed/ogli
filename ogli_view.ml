...
struct
  ...
  (* For optional params: *)
  let none p = option_may (fun _ -> set p None) p.value
end
...
let delete t s =
  ...
  (* Render the whole tree (FIXME: not what we are going to redraw later, see
   * later commands in cmds ; actually Del cmds should be replaced by Adds with
   * bbox in the cmds list earlier) in the shape bbox: *)
...
let render t =
  ...
  (* cmds are here in the reverse painting order *)
...
(* tries to minimize the amount of work generated by moving the pointer *)
let last_move_event = ref 0.
  ...
  let on_event event pos =
    ...
    try
      ...
      and loop tree =
        ...
        | Some (Shape s, _) when (event = Click || event = ShiftClick) && s.on_sub_click <> None ->
            (try look_for_handler tree
            with Exit -> (* click was handled *)
                Lr44.option_may (fun f -> f (event = ShiftClick) pos) s.on_sub_click ;
                raise Exit)
        | _ -> (* just recurse then *)
